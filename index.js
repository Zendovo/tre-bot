...
bot.on("message", async message => {
  ...
  if (cmd == `${prefix}setmessagechannel`) {
    ...
    var chid = args[0];
    con.query(`SELECT * FROM trebotconfig WHERE name = 'messagechannel'`, (err, rows) => {
      let sql = `UPDATE trebotconfig SET value = '${chid}' WHERE name = 'messagechannel'`;
  ...
  if (cmd == `${prefix}setjoinmessage`) {
    if (!message.member.roles.find('name', 'Bot Commander')) return message.channel.send(":x: You do not have the permission!");
    var jmsg = args.join(" ");
    con.query(`SELECT * FROM trebotconfig WHERE name = 'joinmessage'`, (err, rows) => {
      let sql = `UPDATE trebotconfig SET value = '${jmsg}' WHERE name = 'joinmessage'`;
      con.query(sql, console.log);
    });
    con.query("SELECT * FROM trebotconfig WHERE name = 'joinmessage'", (err, rows) => {
      if(err) throw err;
  ...
  if (cmd == `${prefix}setleavemessage`) {
    if (!message.member.roles.find('name', 'Bot Commander')) return message.channel.send(":x: You do not have the permission!");
    var lmsg = args.join(" ");
    con.query(`SELECT * FROM trebotconfig WHERE name = 'leavemessage'`, (err, rows) => {
      let sql = `UPDATE trebotconfig SET value = '${lmsg}' WHERE name = 'leavemessage'`;
      con.query(sql, console.log);
    });
    con.query("SELECT * FROM trebotconfig WHERE name = 'leavemessage'", (err, rows) => {
      if(err) throw err;
  ...
  if (cmd == `${prefix}setdmmessage`) {
    if (!message.member.roles.find('name', 'Bot Commander')) return message.channel.send(":x: You do not have the permission!");
    var dmmsg = args.join(" ");
    con.query(`SELECT * FROM trebotconfig WHERE name = 'dmmessage'`, (err, rows) => {
      let sql = `UPDATE trebotconfig SET value = '${dmmsg}' WHERE name = 'dmmessage'`;
      con.query(sql, console.log);
    });
    con.query("SELECT * FROM trebotconfig WHERE name = 'dmmessage'", (err, rows) => {
      if(err) throw err;
